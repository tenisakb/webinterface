...
var getHashFromChunkNumber = function(section_width, chunk_number, indexHashes, cache){
	...
	if(hash == NOT_IN_CACHE_STATUS){
		...
		for (var chunk_id in cache) {
			if(counter > keepCount){
				if (cache.hasOwnProperty(chunk_id)) {
					delete cache[chunk_id];
				}
